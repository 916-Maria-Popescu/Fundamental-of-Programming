...
def sequence_7 (list):
    """"Display the longest seq of the consecutive complex nrs whose difference between modules is a prime number."""
    max = 0
    now = 0
    k = 0
    for i in range(1, len(list)):
          if int(calc_dif_mod_cn(get(list, i), get(list, i-1))) != 0:    # the dif is a prime number
                now = now + 1
          else:
              if (now > max):
                  max = now
...
def sum_10r10i (list):
    ...
    for i in range (0, len(list)):
        ...
        for j in range (i, len(list)):
           sum = sum + get(list,j)
           if sum == complex(10,10):
               if max < j-i + 1:
                   max = j-i + 1
                   last = j
    first = last - max + 1
    for i in range (first, last+1):
        req_list.append(get(list ,i))
...
if __name__ == '__main__':
    ...
    import cmath
    x = 0
    list_clx_nr = [complex(10, 10), complex(0, 1), complex(8, 0), complex(2, 10), complex(3, 5), complex(3, 0),
                            complex(1, 2), complex(0, 0), complex(9, 9), complex(1, 1)]
    while True:
         x = int(print_menu())
         if x == 5:
             break
             ...
             print ("The longest sequence of the complex numbers whose sum is 10 + 10i:")
             print(sum_10r10i(list_clx_nr))